          ...
          <tr>
            ...
            ${cells.map(cell => {
              ...
              return `<td class="shift-cell${isSun?' sun-cell':''}">
                <div class="shift-pill" style="background:${st.bgColor};border:1.5px solid ${st.color}40;">
                  ...
                  ${sh.hours > 0 ? `<span class="shift-hours" style="color:${st.color};">${sh.hours}h${sh.breakH>0?` (-${sh.breakH}h)`:''}</span>` : ''}
                  ${sh.note ? `<span style="font-size:10px;color:${st.color};opacity:.6;font-style:italic;">${sh.note}</span>` : ''}
                </div>
              </td>`;
            }).join('')}
            <td class="total-cell">
